...
namespace ExitGames.Client.Photon
{
	internal class NConnect
	{
		...
		private sealed class _003C_003Ec__DisplayClass3
		{
			...
			public NConnect _003C_003E4__this;

			public void _003CRun_003Eb__0()
			{
				_003C_003E4__this.peer.ReceiveIncomingCommands(inBuffer, inBuffer.Length);
			}
		}
		...
		internal NConnect(EnetPeer npeer)
		{
			...
			peer = npeer;
			int num = peer.ServerAddress.IndexOf(':');
			string text = peer.ServerAddress.Substring(0, num);
			serverPort = short.Parse(peer.ServerAddress.Substring(num + 1));
		...
		public void Run()
		{
			...
			while (!obsolete)
			{
				try
				{
					...
					lock (syncer)
					{
						_003C_003Ec__DisplayClass.inBuffer = new byte[sock.Available];
						sock.Receive(_003C_003Ec__DisplayClass.inBuffer);
					}
					if (peer.NetworkSimulationSettings.IsSimulationEnabled)
					{
						...
						if (myAction == null)
						{
							myAction = _003C_003Ec__DisplayClass._003CRun_003Eb__0;
						}
						enetPeer.ReceiveNetworkSimulated(myAction);
					}
					else
					{
						peer.ReceiveIncomingCommands(_003C_003Ec__DisplayClass.inBuffer, _003C_003Ec__DisplayClass.inBuffer.Length);
					}
				}
				catch (Exception ex)
				{
			...
			isRunning = false;
			obsolete = true;
			peer.EnqueueActionForDispatch(_003CRun_003Eb__1);
		}
