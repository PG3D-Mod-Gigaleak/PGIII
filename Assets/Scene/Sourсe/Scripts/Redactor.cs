...
public class Redactor : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (showEnabled && !touchEnabled && Input2.touchCount == 0)
		{
			touchEnabled = true;
		}
		if (!showEnabled || !touchEnabled)
		...
		{
			Vector2 point = new Vector2(Input2.touches[0].position.x, Input2.touches[0].position.y);
			...
			if (Input2.touches[0].phase == TouchPhase.Ended && saveToHistory)
			{
